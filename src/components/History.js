...
export default function History() {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
      if (user) {
        ...
        firebase
          ...
          .child(uid)
          .child("date")
          .child(parseDailyData(Number(ch_date)))
          .get()
          .then((snapshot) => {
          ...
          .finally(() =>
            setLoading((prev) => ({ ...prev, searchDate: false }))
          );
  ...
  const _shouldRenderLoading =
    loading.goal || loading.initial || loading.searchDate || loading.today;
          ...
          <Stack spacing={6} display="flex" direction="row">
            <Box
              ...
              sx={{
                ...
                height: 500,
                p: 2,
                boxShadow: "1px 1px 1px 0.5px ",
                borderRadius: 2,
                bgcolor: "#FCFCFC"
              }}>
              {_shouldRenderLoading && (
                <CircularProgress
                  style={{ marginLeft: "47%", marginTop: "37%" }}
                />
              )}
              {!_shouldRenderLoading && (
            ...
            <Stack spacing={4} display="flex" direction="column">
              <Box
                ...
                {_shouldRenderLoading && (
                  <CircularProgress
                    style={{ marginLeft: "47%", marginTop: "25%" }}
                  />
                )}
                {!_shouldRenderLoading && (
                  <div style={{ textAlign: "center" }}>
