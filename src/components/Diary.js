...
export default function Diary() {
    ...
    const notify = () => toast.success('Diary day added.', {
        position: "top-center",
        autoClose: 3000,
    ...
    const addDiary = async (e) => {
        ...
        try {
            ...
            database_ref.child('users/' + user.uid).child("date").child(today).update(diary_data);
            //alert("Day updated.")
            notify();
            this.render();
    ...
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            ...
            firebase.database().ref('users').child(uid).child("date").child(parseDailyData(Number(date))).get()
                .then((snapshot) => {
            ...
            <form>
                <div className="question">
                    ...
                    <Box display='table' sx={{
                        ...
                        />
                        <p></p>
                        <Button onClick={() => clickButton()} variant="contained" style={{ background: '#e5155c' }} className="button-daily" startIcon={<BorderColor />}> Submit Diary </Button>
                    </Box>
                </div>
            </form>
            <br></br> <hr></hr> <br></br>
